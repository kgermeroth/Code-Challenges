...
class CircularArray(object):
    ...
    def add_item(self, item):
        ...
        self.current_rotation.append(item)


    def get_by_index(self, index):
        """Return the data at a particular index."""

        if index > len(self.current_rotation):
            return None

        return self.current_rotation[index]
    ...
    def rotate(self, increment):
        ...
        if abs(increment) > len(self.current_rotation):
            # if value is less than 0 than % does not work the same. Convert to a positive, do mod, convert back
            if increment < 0:
                increment = -(-increment % len(self.current_rotation))
            else:
                increment = (increment % len(self.current_rotation))

        # the new rotation will be the items from the index equivalent to the increment through the end of the list, 
        # and then the beginning of the list up until the increment index

        new_rotation = self.current_rotation[increment:] + self.current_rotation[:increment]
    ...
    def print_array(self):
        ...
        for item in self.current_rotation:
            print(item)
